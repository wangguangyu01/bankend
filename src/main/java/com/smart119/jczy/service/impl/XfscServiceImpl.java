...
public class XfscServiceImpl implements XfscService {
	...
	public int remove(String xfscTywysbm){
		return xfscDao.updateStatus(xfscTywysbm);
	}

	@Override
	public boolean batchRemove(String[] xfscTywysbms){
		int count = 0;
		for (String xfscTywysbm :xfscTywysbms) {
			xfscDao.updateStatus(xfscTywysbm);
			count++;
		}
		if (count == xfscTywysbms.length) {
			return true;
		} else {
			return false;
		}
