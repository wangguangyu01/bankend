    ...
    public int update(XfjyryDO xfjyry) {
        ...
        if (StringUtils.isNotBlank(xfjyry.getUserid())) {
            ...
            if (!ObjectUtils.isEmpty(user)) {
                if (StringUtils.isNotBlank(xfjyry.getUsername()) &&
                        !StringUtils.equals(user.getUsername(), xfjyry.getUsername())) {
                    //验证是否重复
					...
					// 密码
                    user.setPassword(MD5Utils.encrypt(username, xfjyry.getPassword()));
                }
                //角色
