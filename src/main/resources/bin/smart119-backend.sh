...
log_file=${LOG_FILE-"$APP_DIR/logs/smart119_backend.log"}
...
start(){
    ...
    if [[ $APP_CONF ]]; then
        cd $APP_DIR/bin &&
        nohup java -jar $APP_DIR/"$APP_FILE" --spring.config.location="$APP_CONF" > $APP_DIR/log/smart119_backend.log 2>&1 &
		'starting...'
    else
	echo 'starting...'
	cd $APP_DIR/bin &&
